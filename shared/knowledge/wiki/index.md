---
title: "知识库总索引"
date_modified: 2026-05-03
total_pages: 19

# 知识库总索引

## 总览

本项目知识库覆盖：AI数字短剧自动化、TK东南亚运营、LLM Wiki方法论、会话归档机制、知识库自动化运维。

## 最近新增

- 2026-05-03: 环境分离体系 — GitHub test/main 双分支 · 页面正式/测试标签 · :5002/:5001 端口隔离
- 2026-05-03: v3.7.21 — 删图/重绘API路由修复 · 环境标签 · GitHub test分支
- 2026-05-03: v3.7.20 — DM-6/7质量可视化 + 管线日志结构化 + DM-1角色图删图/重绘 + 🎤→DM-3管线串联 + 14项审计发现
- 2026-05-03: SAFETY_RULES.md v1.2 — 环境隔离 + 精准代码编辑 + 同步审批闭环

## 概念（2 篇）

- [[llm-wiki-methodology]] — 基于 Karpathy 模式的 raw + wiki + CLAUDE.md 知识库架构
- [[session-archiving]] — 基于 Clicky/CPR 设计理念的会话归档机制

## 实体（1 篇）

- [[agentic-os-v3.6-architecture]] — Agentic OS v3.6 完整架构 · 30端点 · 6集管线 · 25张ComfyUI渲染

## 工具脚本（5 篇）

- `shared/scripts/wiki-ingest.py` — raw→wiki 自动编译
- `shared/scripts/wiki-query.py` — 回答归档到 output
- `shared/scripts/wiki-lint.py` — 每周健康检查
- `scripts/archive-session.sh` — 会话一键归档
- `scripts/setup-lint-cron.sh` — 安装 lint 定时任务

## 模板（4 篇）

- `shared/knowledge/templates/concept.md`
- `shared/knowledge/templates/entity.md`
- `shared/knowledge/templates/source.md`
- `shared/knowledge/templates/output.md`

## 综合分析（1 篇）

- `wiki/syntheses/` — (待填充)

## 最近新增
1. [2026-05-01] v3.7.2 DM-1角色面板恢复+视频预览 — renderDM1从git历史完整恢复(268行)+🎥✏️视频提示词交互 (commit c3f3be7)

1. [2026-05-01] v3.7 Dashboard修复 + TK管线对齐 — 2xReferenceError + SyntaxError修复, 5个TK milestone completed零mock (commit 080bc1c)

1. [2026-05-01] v3.6.27 诚实收尾 — visual_bible修复+reReviewDM0+铁则强化+全量推送 (commit ef82f68)

1. [2026-05-01] PRD v3.6.20 全量修订 — 音色克隆+视频提示词+108将全角色可视化 (commit 59c7ba4)
1. [2026-04-30] [[agentic-os-v3.6-architecture]] — v3.6.1架构实体 · QA 80/80 · GitHub df10af7→240a068
2. [2026-04-27] wiki-ingest.py（脚本）
3. [2026-04-27] wiki-query.py（脚本）
4. [2026-04-27] wiki-lint.py（脚本）
5. [2026-04-27] templates/（模板）
6. [2026-04-27] wiki-ops skill（技能）
7. [2026-04-27] [[llm-wiki-methodology]]（概念）
8. [2026-04-27] [[session-archiving]]（概念）

## 概念

- [[test-raw-note]] — Ingested from test-raw-note.md
- [[miaoshou-erp-api]] — Miaoshou ERP API documentation (login, shops, collection box, pending publish API)
