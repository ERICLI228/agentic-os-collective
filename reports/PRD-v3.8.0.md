...
> **文档状态**: ✅ v3.7.30 — 14项审计全量清零 (P0×4 + P1×4 + P2×6) + 剧集多选决策 + 全量同步管线
> **前置版本**: v3.7.21 (2026-05-03)
...
| v3.7.22 | 05-03 | OpenCode + 阿牛 | P0-Z: mock/real数据标识统一 (DATA_BADGE + mock-section高亮 + toggle) | 🚀 |
| v3.7.23 | 05-03 | OpenCode + 阿牛 | P0-V: DM-6/7视频播放器 (<video> + /api/video/<ep>端点 + CSS + JS handlers) | 🚀 |
| v3.7.24 | 05-03 | OpenCode + 阿牛 | P0-A: DM-0原稿vs AI评审左右分栏对照 (.dm0-side-by-side grid) | 🚀 |
| v3.7.25 | 05-03 | OpenCode + 阿牛 | P0-Y: 导演总览 (管线进度+剧集状态+关键指标+快捷跳转) | 🚀 |
| v3.7.26 | 05-03 | OpenCode + 阿牛 | P1: charMap动态加载(DM-3) + 剧集过滤(DM-2) + mock警告 + 渲染队列(DM-4) | 🚀 |
| v3.7.27 | 05-03 | OpenCode + 阿牛 | P2: 镜头参数(K) + 人物关系图(G) + 排播扩展EP01~06(W) | 🚀 |
...
| v3.7.18~v3.7.21 | DM-6/7质量可视化 · 删图/重绘API · 环境标签 · 3端分离 · 14项审计发现 |
| v3.7.22~v3.7.25 | P0: mock标识(Z) + 视频播放(V) + 左右分栏(A) + 导演总览(Y) |
| v3.7.26 | P1: charMap动态加载 + 剧集过滤 + mock警告 + 渲染队列 |
| v3.7.27~v3.7.28 | P2: 镜头参数 + 关系图 + 排播扩展 + 角色映射 + 台词对照表 + 本地fallback |
| v3.7.29 | DM-0剧集多选决策 (复选框+全选+已选N/6计数+联动) |
| v3.7.30 | DM-0全量同步管线 (决策→轮询→原稿→故事板→TXT/SRT联动刷新) |
