...
if __name__ == "__main__":
    ...
    if TASK_ID:
        # 配音合成结果
        audio_files = [
            {"scene": "武松打虎", "voice": "Alvaro", "duration": "45s", "file": "wusong_ep01.wav"},
            {"scene": "小二劝酒", "voice": "Eric", "duration": "12s", "file": "xiaoer_ep01.wav"},
            {"scene": "老虎吼叫", "voice": "Brian", "duration": "8s", "file": "laohu_ep01.wav"}
        ]
        
        try:
            subprocess.run([
                'python3', str(Path.home() / '.openclaw/core/task_updater.py'),
                TASK_ID, 'MS-7', 'completed'
            ], timeout=10)
            ...
            print(f"✅ 已更新任务 {TASK_ID} 里程碑 MS-7 -> completed")
            print(f"✅ 产出内容已写入: 配音合成")
        except Exception as e:
            print(f"⚠️ 里程碑更新失败: {e}")