    ...
    if TASK_ID:
        try:
            ...
            if task_file.exists():
                ...
                controversy_summary = """
检测到以下潜在争议点:
1. 武松打虎情节过于暴力 - 建议改为武松用智慧吓退老虎
2. 血腥描写比例过高 - 建议减少血腥细节描写
3. 老虎形象过于凶猛 - 可增加幽默元素平衡
...
• 增加幽默对话缓解紧张氛围
                    ...
                    for m in task.get('milestones', []):
                        if m.get('id') == 'MS-2':
                            ...
                            m['execution_details']['output_content'] = {
                                ...
                                'artifacts': [
                                    {'name': 'controversy_report.md', 'path': f'~/.openclaw/skills/water-margin-drama/output/{TASK_ID}_MS-2.txt'}
                                ],
                            ...
                            m['decision_options'] = ['修改', '保持']
                            m['decision_deadline'] = (datetime.now() + timedelta(hours=24)).isoformat()
            ...
            print(f"✅ 已更新任务 {TASK_ID} 里程碑 MS-2 -> completed")
            print(f"✅ 产出内容已写入: 争议点分析报告")
